---
title: "Mamadou Dicko"
date: "2019-11-05T15:45:05+01:00"
draft: false
description: "L'ÉCOLOGIE : UNE URGENCE VITALE, UNE CHANCE HISTORIQUE !"
slug: "Mamadou"
weight: 40

card:
  image: "images/mamadou-logo-thumb.jpg"
  fonction: "Membre du CA"
  membre: "Membre du CA"
  presentation: "Membre de Leidimen depuis sa création"
  ville: "Bondoufle"
  pays: "France"
  email: ""
  tel: ""
  devise: "L'ÉCOLOGIE : UNE URGENCE VITALE, UNE CHANCE HISTORIQUE !"
  specialites:
    - "Écologie"
    - "Développement durable"
    - "Engagement citoyen"
    - "Action associative"

experience:
  - date: "2019"
    title: "Membre fondateur de Leidimen"
    description: "Co-fondation de l'association Leidimen pour promouvoir la solidarité entre Bondoufle et le Mali"
    category: "Associatif"
    icon: "bi-heart-fill"
    achievements:
      - "Participation à la création de l'association"
      - "Membre actif du conseil d'administration"
      - "Engagement dans les projets de développement"
    skills:
      - "Gestion associative"
      - "Engagement solidaire"
      - "Travail en équipe"

  - date: "2019-2025"
    title: "Actions écologiques et durables"
    description: "Promotion de l'écologie et du développement durable dans les projets de l'association"
    category: "Environnement"
    icon: "bi-tree-fill"
    achievements:
      - "Sensibilisation aux enjeux écologiques"
      - "Intégration de pratiques durables"
      - "Promotion de l'éco-responsabilité"
    skills:
      - "Écologie"
      - "Développement durable"
      - "Sensibilisation"
      - "Innovation verte"

  - date: "2020-2025"
    title: "Engagement citoyen et communautaire"
    description: "Participation active aux événements et initiatives de l'association"
    category: "Associatif"
    icon: "bi-people-fill"
    achievements:
      - "Organisation d'événements solidaires"
      - "Mobilisation de la communauté"
      - "Renforcement des liens France-Mali"
    skills:
      - "Action citoyenne"
      - "Organisation événementielle"
      - "Relations communautaires"

categories:
  - "leidimen"
  - "CA"
  - "fondateur"

tags:
  - "Bondoufle"
  - "Mali"
  - "écologie"
  - "développement durable"
---

{{< myimage src="images/mamadou-logo.jpg" >}}

## Mamadou Dicko - Membre fondateur de Leidimen

Mamadou Dicko est membre fondateur et membre du conseil d'administration de l'association Leidimen depuis 2019. Son engagement est guidé par une conviction profonde : **"L'ÉCOLOGIE : UNE URGENCE VITALE, UNE CHANCE HISTORIQUE !"**

### Engagement écologique

Fervent défenseur de l'écologie et du développement durable, Mamadou œuvre pour intégrer ces valeurs dans tous les projets de l'association. Il sensibilise la communauté aux enjeux environnementaux et promeut des pratiques éco-responsables.

### Action solidaire

Membre actif du conseil d'administration, Mamadou participe à l'organisation d'événements solidaires et au renforcement des liens entre Bondoufle et le Mali. Son engagement citoyen inspire et mobilise la communauté autour des projets de développement.


