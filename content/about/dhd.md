---
title: "Hamassala Dicko"
date: "2006-10-25T22:35:49+01:00"
draft: false
description: "La solidarité au coeur de mes préoccupations - Trésorière de Leidimen"
slug: "David"
weight: 2

card:
  image: "images/david-logo-thumb.jpg"
  fonction: "Trésorière"
  membre: "Membre du CA"
  presentation: "Trésorier de Leidimen"
  ville: "Paris"
  pays: "France"
  email: "david.dicko@menopresto.net"
  tel: ""
  devise: "Liberté - Égalité - Fraternité et Solidarité"
  specialites:
    - "Gestion financière"
    - "Comptabilité associative"
    - "Suivi budgétaire"
    - "donateur"

experience:
  - date: "2019"
    title: "Trésorier de Leidimen"
    description: "Gestion financière de l'association Leidimen, suivi des comptes et des budgets des projets"
    category: "Associatif"
    icon: "bi-calculator-fill"
    achievements:
      - "Gestion de la comptabilité de l'association"
      - "Suivi des budgets des projets"
      - "Préparation des bilans financiers annuels"
    skills:
      - "Comptabilité associative"
      - "Gestion budgétaire"
      - "Reporting financier"
      - "Conformité administrative"

  - date: "2019-2024"
    title: "Gestion des projets éducatifs"
    description: "Suivi financier des projets de soutien à l'éducation au Mali"
    category: "Éducation"
    icon: "bi-coin"
    achievements:
      - "Gestion des fonds pour les projets scolaires"
      - "Optimisation des dépenses"
      - "Transparence financière"
    skills:
      - "Gestion de projet"
      - "Suivi financier"
      - "Allocation de ressources"

  - date: "2020-2024"
    title: "Membre fondateur actif"
    description: "Participation active aux activités de l'association et aux événements culturels"
    category: "Associatif"
    icon: "bi-heart-fill"
    achievements:
      - "Participation aux événements de collecte"
      - "Support aux initiatives culturelles"
      - "Renforcement des liens communautaires"
    skills:
      - "Engagement associatif"
      - "Organisation d'événements"
      - "Relations communautaires"

categories:
  - "leidimen"
  - "CA"
  - "bureau"
  - "fondateur"


tags:
  - "Leidimen"
  - "trésorier"
  - "Mali"
---

{{< myimage src="images/david-logo.jpg" >}}

{{< details "A propos de moi" >}}
{{< typeit
  tag=h1
  lifeLike=true
>}}
Hamassala Hamadoun Pathe Ameri Diona Dicko
{{< /typeit >}}

## Trésorier de Leidimen

Hamassala Dicko est trésorier et membre fondateur de l'association Leidimen. il assure la gestion financière de l'association avec rigueur et transparence, garantissant le bon usage des fonds collectés pour les projets de solidarité.

Sa devise : **"Liberté - Égalité - Fraternité et Solidarité"** guide son engagement au service du développement et de l'éducation dans les villages maliens.
{{< /details >}}