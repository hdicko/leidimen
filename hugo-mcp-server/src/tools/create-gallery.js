...
export async function createGallery(hugoRoot, params) {
	const galleryDir = path.join(hugoRoot, 'content', 'galleries', params.slug);
	...
	if (params.villages?.length) {
		lines.push(`villages:`);
		params.villages.forEach((v) => lines.push(`  - "${v}"`));
	}
	lines.push('---');
	lines.push('');
	lines.push('{{< gallery >}}');
	lines.push('');
	lines.push('<!--');
	lines.push('  Add images to this folder (content/galleries/' + params.slug + '/)');
	lines.push('  Supported formats: .jpg, .jpeg, .png, .webp');
	lines.push('  The gallery shortcode will auto-detect page resources.');
	lines.push('-->');
	lines.push('');
